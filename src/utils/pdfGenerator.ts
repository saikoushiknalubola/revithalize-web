...
export const generateBatteryReportPDF = (
  ...
  try {
    ...
    // Usage data table using autoTable
    autoTable(doc, {
      startY: yPos + 122,
...
// Enhanced compliance report generator
export const generateComplianceReportPDF = (complianceData: any, userName: string): Blob => {
  try {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    const today = new Date().toLocaleDateString('en-GB');
    ...
    doc.setFontSize(10);
    doc.text(`Generated: ${today} | For: ${userName}`, pageWidth / 2, 30, { align: 'center' });
    ...
    autoTable(doc, {
      startY: 60,
      head: [['Category', 'Status', 'Last Checked', 'Next Due']],
      body: complianceData.items?.map((item: any) => [
        item.category || 'N/A',
        item.status || 'Unknown',
        item.lastChecked || 'N/A',
        item.nextDue || 'N/A'
      ]) || [['No data available', '', '', '']],
      theme: 'grid',
      styles: { fontSize: 9 },
      headStyles: { fillColor: [0, 100, 120] }
    });
    ...
    const doc = new jsPDF();
    doc.text('Error generating compliance report. Please try again.', 20, 20);
    return doc.output('blob');
  }
};

// Enhanced analytics report generator
export const generateAdvancedAnalyticsReportPDF = (analyticsData: any, userName: string): Blob => {
  try {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    const today = new Date().toLocaleDateString('en-GB');
    ...
    doc.setFontSize(20);
    doc.setTextColor(0, 128, 0);
    doc.text('ADVANCED ANALYTICS REPORT', pageWidth / 2, 20, { align: 'center' });
    ...
    doc.setFontSize(14);
    doc.setTextColor(40, 40, 40);
    doc.text('Analytics Overview', 20, 50);
    
    doc.setFontSize(10);
    doc.text('Performance Metrics:', 20, 65);
    doc.text(`• Energy Efficiency: ${analyticsData.efficiency || '91'}%`, 25, 75);
    doc.text(`• Battery Health: ${analyticsData.batteryHealth || '97'}%`, 25, 85);
    doc.text(`• Range Optimization: ${analyticsData.rangeOpt || '87'}%`, 25, 95);
    doc.text(`• Cost Savings: ₹${analyticsData.costSavings || '12,450'}`, 25, 105);

    return doc.output('blob');
  } catch (error) {
    console.error('Error generating analytics PDF:', error);
    const doc = new jsPDF();
    doc.text('Error generating analytics report. Please try again.', 20, 20);
    return doc.output('blob');
  }
};
...
export const generateTicketPDF = (ticket: ServiceTicket, userName: string, vehicleDetails: any): Blob => {
  try {
    ...
    const doc = new jsPDF();
    doc.text('Error generating ticket. Please try again.', 20, 20);
    return doc.output('blob');
  }
};
